...
<h2 id="devices"><a href="#devices">Devices</a></h2>
...
-   Each folder has an ID.<FootnoteRef name="2" />
...
<h2 id="closing-remarks"><a href="#closing-remarks">Closing remarks</a></h2>
...
For those who heavily customize Syncthing, there are other NixOS options available to tweak. You can find them [in the NixOS Search for options](https://search.nixos.org/options?channel=unstable&from=0&size=50&sort=relevance&type=packages&query=syncthing). Additionally, if you are trying to migrate an existing Syncthing configuration to Nix, then you can check out your config file at `~/.config/syncthing/config.xml`.<FootnoteRef name="3" /> With this, you can see the value of every Syncthing setting (although the file will, of course, be in JSON format).
