...
date: 2022-06-14T18:52:00-05:00
---

# Table of Contents

1.  [Summary](#summary)
2.  [Helpful sources](#helpful-sources)
3.  [What are citation systems?](#what-are-citation-systems)
4.  [Org-cite](#org-cite)
    1.  [Understanding `org-cite` export processors](#understanding-org-cite-export-processors)
    2.  [Which processor should I use?](#which-processor-should-i-use)
    3.  [Configuration](#configuration)
    4.  [Usage](#usage)
5.  [Citar](#citar)
    1.  [Capturing (Outdated)](#capturing-outdated)
    2.  [Advising `citar-org-update-pre-suffix` (Outdated)](#h:20241106T044538.716623)
    3.  [Aesthetics](#aesthetics):ATTACH:
6.  [Other interesting packages](#other-interesting-packages)
    1.  [Zotero](#zotero)
    2.  [Embark](#embark)
    3.  [Org-roam-bibtex](#org-roam-bibtex)
    4.  [Citar-capf](#citar-capf)
    5.  [Org-ref](#org-ref)
...
<div>
<span class="timestamp-wrapper"><span class="timestamp">[2023-07-20 Thu]</span></span> Readers should be aware that some package-specific information in this article may be outdated. Thus, readers are advised from outright copying code in this article without verifying (e.g. reading docstrings) the intended behavior. Nevertheless, the concepts related to `org-cite` and citation management should remain relevant and insightful.
</div>
...
<div>
flowchart LR
A((Plain-text)) &mdash; B&#123;Org-export&#125;
B ===&gt; C
B ===&gt;D
B ===&gt; E

subgraph CSL Processor
C&#123;&#123;HTML, MD, ODT, plain-text, etc.&#125;&#125;
D[LaTeX]
D -.-&gt;|Compile with citations as rendered text| F(PDF)
end

subgraph Bibtex and Natbib processor
E[LaTeX]
E -.-&gt;|Compile with citations as LaTeX commands|G(PDF)
end
</div>
...
*Otherwise*, use the `csl` processor<sup><a id="fnr.2" class="footref" href="#fn.2" role="doc-backlink">2</a></sup> (assuming you have a `.csl` file for your desired citation style).
...
2.  `#+PRINT_BIBLIOGRAPHY:` within a document to mark where the bibliography/references/works cited should be.<sup><a id="fnr.6" class="footref" href="#fn.6" role="doc-backlink">6</a></sup>
...
You can then set keybinds for common `citar` commands. For example, in the `:general` use-package keyword:<sup><a id="fnr.7" class="footref" href="#fn.7" role="doc-backlink">7</a></sup>
...
<div>
<span class="timestamp-wrapper"><span class="timestamp">[2023-08-19 Sat]</span></span> The [citar-org-roam](https://github.com/emacs-citar/citar-org-roam) package provides all that is mentioned in this section and more. That package should be used instead in any instance of a desired integration between `org-roam` and `citar`.
</div>

If you use `org-roam`, then the following is how you select bibliographic source and create an `org-roam` node from it. All credit goes to [Jethro Kuan](https://www.jethro.dev/), the creator of `org-roam`, though I've made a few of my own customizations:<sup><a id="fnr.8" class="footref" href="#fn.8" role="doc-backlink">8</a></sup>
...
I add it to an accessible keybinding:<sup><a id="fnr.9" class="footref" href="#fn.9" role="doc-backlink">9</a></sup>
...
1.  Temporarily enable [typo-mode](https://github.com/jorgenschaefer/typoel).<sup><a id="fnr.10" class="footref" href="#fn.10" role="doc-backlink">10</a></sup>
...
One can also add this command to `citar-citation-map`.<sup><a id="fnr.11" class="footref" href="#fn.11" role="doc-backlink">11</a></sup>
...
![img](assets/2022-06-15_12-52-26_screenshot.png "Visual improvements to `citar`'s minibuffer interface.")
...
[Citar-capf](https://github.com/mclear-tools/citar-capf) provides a `complation-at-point-function`<sup><a id="fnr.12" class="footref" href="#fn.12" role="doc-backlink">12</a></sup> for citations. However, if you're reading this in the future, there's a likely chance that [this commit](https://github.com/emacs-citar/citar/commit/784791a090f9a2ed293f8b3ebd5636c690b718bc) would have been merged. The commit merges `citar-capf` with `citar`.
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2022-06-15 Wed]</span></span>
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2022-06-19 Sun]</span></span>
...
-   <span class="timestamp-wrapper"><span class="timestamp">[2023-07-20 Thu]</span></span>
    -   Noted that some package-specific information in this article may be outdated.
    -   Noted update to `citar-org-update-pre-suffix`
-   <span class="timestamp-wrapper"><span class="timestamp">[2023-08-19 Sat]</span></span>
...
<sup><a id="fn.1" href="#fnr.1">1</a></sup> The `basic` processor provides bare functionality and should never be preferred over the alternative, if possible.

<sup><a id="fn.2" href="#fnr.2">2</a></sup> See [Configuration](#configuration).

<sup><a id="fn.3" href="#fnr.3">3</a></sup> This is an `org-export` backend. This means that, for instance, `md` refers to the `org-export` `md` backend as well as any other derived backends. Read the documentation for `org-export-define-derived-backend`.

<sup><a id="fn.4" href="#fnr.4">4</a></sup> Notice that a citation style is required for the `csl` processor, and not for the `bibtex` processor.

<sup><a id="fn.5" href="#fnr.5">5</a></sup> Be aware that I use `use-package`'s `:custom-face` keyword to redefine these two faces. One can just as easily use the built-in `defface` of `set-face-attribute`.

<sup><a id="fn.6" href="#fnr.6">6</a></sup> Also see [this section of tecosaur's guide](https://blog.tecosaur.com/tmio/2021-07-31-citations.html#basic-usage).

<sup><a id="fn.7" href="#fnr.7">7</a></sup> You can also just use the built-in `define-key`.

<sup><a id="fn.8" href="#fnr.8">8</a></sup> This assumes that you have `org-roam-bibtex` installed and enabled. See [Org-roam-bibtex](#org-roam-bibtex).

<sup><a id="fn.9" href="#fnr.9">9</a></sup> `kb/note-keys` is my own `general.el` leader key. I've mapped its prefix to `C-c n`.

<sup><a id="fn.10" href="#fnr.10">10</a></sup> I prefer directly inserting curly quotes, em-dashes, and en-dashes into my buffers. I enable `typo-mode` here so that typing page ranges properly inserts en-dashes.

<sup><a id="fn.11" href="#fnr.11">11</a></sup> See [Embark](#embark).

<sup><a id="fn.12" href="#fnr.12">12</a></sup> Also see [Vertico, Marginalia, All-the-icons-completion, and Orderless](20221222T011801--vertico-marginalia-all-the-icons-completion-and-orderless__guides.md).
