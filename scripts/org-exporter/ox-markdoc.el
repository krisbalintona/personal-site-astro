;;; ox-markdoc.el --- Astro Markdoc Org-mode exporter -*- lexical-binding: t -*-
...
;; Export from Org-mode to Markdoc.  This file defines the org export
...
(defconst org-markdoc-root-dir (project-root (project-current))
  "Root directory of project.
This option is used to define the value of other relevant paths.")

(defconst org-markdoc-content-dir (expand-file-name "src/content/" org-markdoc-root-dir)
  "Directory where all Markdoc content resides.")

(defconst org-markdoc-articles-dir (expand-file-name "articles/" org-markdoc-content-dir)
  "Directory where articles will be exported to.")

(defconst org-markdoc-tags-dir (expand-file-name "tags/" org-markdoc-content-dir)
  "Directory where custom tag pages will be exported to.")

(defconst org-markdoc-standalone-dir (expand-file-name "standalone/" org-markdoc-content-dir)
  "Directory where standalone content will be exported to.")

(defvar org-markdoc-default-alt-text "img"
...
(defun org-markdoc-example-block (example-block _contents info)
  "Transcode EXAMPLE-BLOCK element into Markdoc format.
...
(defun org-markdoc--create-figure (fig caption)
...
(defun org-markdoc-src-block (src-block _contents info)
    ...
    (pcase lang
      ("d2"
       (let* ((caption
              ...
              (info-string
               (let* ((org-markdoc-default-alt-text "diagram")
                      (raw-alt (org-export-read-attribute :attr_markdoc src-block :alt))
                      (alt-text
              ...
              (code-fence
               (format "```%s%s\n%s\n```"
                       ...
                       (if info-string
                         ...
                         "")
                       (string-trim inner))))
         (org-markdoc--create-figure code-fence caption)))
      (_
       (format "```%s\n%s\n```" lang (string-trim inner))))))

(defun org-markdoc-special-block (special-block contents info)
  "Transcode SPECIAL-BLOCK element into Markdoc format.
...
(defun org-markdoc--headline-text-to-slug (headline)
...
(defun org-markdoc-timestamp (timestamp _contents info)
  "Transcode a TIMESTAMP object from Org to Markdoc.
...
(defun org-markdoc--export-get-reference-advice (orig datum info)
...
of these IDs specially for the `markdoc' backend such that they are
...
TYPE-N.  When not using the `markdoc' backend, call
...
how the Markdoc file is processed.  For instance, by default, Astro will
use the Rehype-slug plugin (see https://github.com/rehypejs/rehype-slug)
to generate id attributes for headings, which will overwrite the ones
generated here.

ORIG is the original function.  See `org-export-get-reference' for a
description of DATUM and INFO."
  (if (not (eq (org-export-backend-name (plist-get info :back-end)) 'markdoc))
                 ...
                 (reference
                  (if (org-element-type-p datum 'headline)
                      ...
                      (let* ((slug-cache
                              ...
                              (or (plist-get info :markdoc-headline-slug-cache)
                                  (let ((table (make-hash-table :test #'eq)))
                                    (plist-put info :markdoc-headline-slug-cache table)
                                    table)))
                        ...
                        (or
                         ...
                         (let* ((base-slug (org-markdoc--headline-text-to-slug datum))
                                (slug-counts
                                 ;; Table of BASE-SLUG to count
                                 (or (plist-get info :markdoc-headline-slug-counts)
                                     (let ((table (make-hash-table :test #'equal)))
                                       (plist-put info :markdoc-headline-slug-counts table)
                                       table)))
                    ...
                    (let* ((counters
                            (or (plist-get info :markdoc-display-type-counters)
                                (let ((table (make-hash-table :test #'eq)))
                                  (plist-put info :markdoc-display-type-counters table)
                                  table)))
                           (display-type
...
(advice-add 'org-export-get-reference :around #'org-markdoc--export-get-reference-advice)

(el-patch-defun (el-patch-swap org-md-link org-markdoc-link) (link desc info)
    ...
    (cond
           ...
           (el-patch-swap
             ...
             (let* ((destination-project
                    ...
                    (id
                     ...
                     ;; `org-markdoc--output-directory' for entry IDs, we
                     ;; use that here
                     (format "id=\"%s\""
                             (let* ((cache (or (plist-get info :markdoc-destination-id-cache)
                                               (let ((table (make-hash-table :test #'equal)))
                                                 (plist-put info :markdoc-destination-id-cache table)
                                                 table)))
                                       ...
                                       (let* (result)
                                         (setq buf (find-file-noselect destination-full-path)
                                               result (with-current-buffer buf
                                                        ...
                                                        (file-name-base (org-markdoc--output-directory nil))))
               ...
               (if (not destination-project)
                 ...
                 (format "{%% content-link %s %s%s %%}%s{%% /content-link %%}"
                         collection-name id
     ...
     ((org-export-inline-image-p link org-html-inline-image-rules)
      (let ((path (cond ((not (string-equal type "file"))
            ...
            (el-patch-add
              (alt-text
               (or (org-export-read-attribute :attr_markdoc (org-element-parent-element link) :alt)
                   org-markdoc-default-alt-text)))
            (el-patch-add
              ...
              (path
               (if (plist-get info :output-file)
                   (org-markdoc--copy-attachments raw-path info)
                 path))))
        (el-patch-remove
          (format "![img](%s)"
        ...
        (el-patch-add
          (org-markdoc--create-figure (format "{%% image src=\"%s\" alt=\"%s\" /%%}" path alt-text)
                                      caption))))
     ((string= type "coderef")
...
(defun org-markdoc--frontmatter-quote-string (s)
  "Quote string S for YAML double-quoted scalars.
This function should be used for any object in the YAML frontmatter of a
Markdoc file whose type is a string."
  (concat "\""
          (thread-last s
...
(defun org-markdoc-template (contents info)
...
communication channel for the export process."
  (let* ((entry-type
          (or (plist-get info :markdoc-entry-type)
         ...
         (raw-title (org-markdoc--title-to-utf8 info))
         (escaped-title
          ...
          (when raw-title (org-markdoc--frontmatter-quote-string raw-title)))
         (title (when raw-title (concat "title: " escaped-title)))
         (raw-slug (plist-get info :markdoc-slug))
         (escaped-slug (when raw-slug (org-markdoc--frontmatter-quote-string raw-slug)))
         ...
         (draft (concat "draft: " (plist-get info :markdoc-draft-p))) ; Return YAML boolean
         frontmatter)
    ...
    (pcase entry-type
      ...
      ("articles"
       (let* ((raw-tags (plist-get info :markdoc-tags))
              (parsed-tags
               (when (org-string-nw-p raw-tags)
                 (mapcar #'org-markdoc--frontmatter-quote-string
...
;; Export to buffer
(defun org-markdoc-export-as-markdoc
    (&optional async subtreep visible-only body-only ext-plist)
  "Export current buffer to a Markdoc buffer.
...
Export result is inserted into a buffer named \"*Org Markdoc Export*\",
which is displayed when `org-export-show-temporary-export-buffer' is
non-nil."
  (interactive)
  (org-export-to-buffer 'markdoc "*Org Markdoc Export*"
    async subtreep visible-only body-only ext-plist
    #'markdown-mode))

(defun org-markdoc--interpret-as-ascii (data charset)
...
(defun org-markdoc--title-to-ascii (info)
...
INFO is a plist holding export information."
  (org-markdoc--interpret-as-ascii
   (org-element-interpret-data
    (when (plist-get info :with-title) (plist-get info :title)))
   'ascii))

(defun org-markdoc--title-to-utf8 (info)
...
(defun org-markdoc--title-to-subdirectory-name (title)
...
(defun org-markdoc--output-directory (output-dir &optional subtreep)
...
of the post title passed to `org-markdoc--title-to-subdirectory-name'.
...
directory."
  (let* ((info (org-export-get-environment 'markdoc))
         (title (org-markdoc--title-to-ascii info))
         (date-timestamp (car (plist-get info :date)))
         (directory
          (or
           ...
           (format "%s--%s"
                   ...
                   (org-markdoc--title-to-subdirectory-name title))
           ;; As a fallback, ask user
           (read-file-name "Output directory: " org-markdoc-articles-dir))))
    (expand-file-name directory output-dir)))

(defun org-markdoc--copy-attachments (path info)
...
communication channel."
  (let* ((output-file (plist-get info :output-file))
    ...
    (if (file-exists-p asset-path)
        (message "[org-markdoc] Asset %s already exists, not overwriting" asset-path)
...
(defun org-markdoc--prepare-output-directory (output-directory)
...
`org-markdoc-export-to-markdoc' and `org-markdoc-publish-to-site'."
...
;; Export to file
(defun org-markdoc-export-to-markdoc
    (&optional async subtreep visible-only body-only ext-plist)
  "Export current buffer to a post subdirectory.
The post subdirectory is one calculated by `org-markdoc--output-directory'.
Several files may be created in this directory:
- An \"index.markdoc\", containing the post in Markdoc format.
- An \"assets/\" subdirectory, containing all attachments (see
  `org-markdoc--copy-attachments').
...
Return the output directory's name."
  (interactive)
  (let* ((output-dir (or (plist-get ext-plist :markdoc-output-dir) ; From `org-markdoc-publish-to-site'
                         (org-markdoc--output-directory nil subtreep)))
         (outfile (expand-file-name "index.mdoc" output-dir)))
    (org-markdoc--prepare-output-directory output-dir)
    (org-export-to-file 'markdoc outfile async subtreep visible-only body-only ext-plist)))

;;;;; Define backend

;; For more information about backends and the exporting process, see
;; (org) Advanced Export Configuration.
(org-export-define-derived-backend 'markdoc 'md
  ;; Entry for backend in the org export menu
  :menu-entry
  '(?s "Export to site"
       ((?M "To buffer" org-markdoc-export-as-markdoc)
        (?m "To file" org-markdoc-export-to-markdoc)
        (?o "To file and open"
            (lambda (async subtreep visible-only body-only)
              (if async
                  (org-markdoc-export-to-markdoc t subtreep visible-only body-only)
                (pop-to-buffer
                 (find-file-noselect
                  (org-markdoc-export-to-markdoc nil subtreep visible-only body-only))))))))
  ...
  ;; backend
  :options-alist
  '((:markdoc-slug "MARKDOC_SLUG" nil nil)
    (:markdoc-draft-p "MARKDOC_DRAFT" "markdoc-draft" "true")
    (:markdoc-tags "MARKDOC_TAGS" nil nil space)
  ...
  ;; backend transcoders
  :translate-alist
  '((template . org-markdoc-template)
    (example-block . org-markdoc-example-block)
    (src-block . org-markdoc-src-block)
    (special-block . org-markdoc-special-block)
    (link . org-markdoc-link)
    (timestamp . org-markdoc-timestamp)))

;;;; Org-publish
;; I use org-publish to make it easier to export all my blog posts all
;; together.  The export function `org-markdoc-export-to-markdoc' does all
;; the heavy lifting for export.

(defun org-markdoc-publish-to-site (plist filename pub-dir)
...
`org-publish-project-alist'."
  ...
  ;; `org-markdoc--output-directory' to determine the output file.  So we
  ;; define a modified version of `org-publish-org-to' instead
  (let ((org-inhibit-startup t))
    (org-with-file-buffer filename
      (let* ((output-dir (org-markdoc--output-directory pub-dir)))
        (org-markdoc--prepare-output-directory output-dir)

        (org-markdoc-export-to-markdoc
         nil nil nil (plist-get plist :body-only)
         (org-combine-plists
          plist
          `( :markdoc-output-dir ,output-dir ; Pass to `org-markdoc-export-to-markdoc'
...
(let ((base-options '( :with-toc nil
                       ...
                       :time-stamp-file nil
                       :base-extension "org"
                       :publishing-function org-markdoc-publish-to-site)))
  (setopt
   ...
   `(("standalone"
      ...
      :publishing-directory ,org-markdoc-standalone-dir
      :recursive t
      ,@base-options)
     ("articles"
      :base-directory ,(expand-file-name "articles" krisb-manuscript-blog-directory)
      :publishing-directory ,org-markdoc-articles-dir
      :recursive t
      :markdoc-entry-type "articles"
      ,@base-options)
     ("tags"
      :base-directory ,(expand-file-name "tags" krisb-manuscript-blog-directory)
      :publishing-directory ,org-markdoc-tags-dir
      :recursive t
      :markdoc-entry-type "tags"
      ,@base-options))))

;;; Provide
(provide 'ox-markdoc)
;;; ox-markdoc.el ends here
